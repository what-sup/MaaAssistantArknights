...
截至 MAA v6.16.8 為止，支援自動偵測的模擬器及其連線位址如下：
...
前往模擬器的安裝路徑。在 Windows 系統中，您可以在模擬器執行時，於工作管理員對該處理程序點擊右鍵，選擇 `開啟檔案所在位置`。
...
- 方案 2 ：尋找已建立的 ADB 連線
  ...
  3. 切換到 `網路` 索引標籤，在 `接聽連接埠` 的名稱欄位中尋找模擬器處理程序名稱（例如 `HD-Player.exe`）。
  4. 記錄模擬器處理程序所有的接聽連接埠。
...
請選擇對應模擬器的配置，若清單中沒有則選擇「通用模式」。若通用模式不可用，請嘗試並選擇其他任一可用的配置。
...
2. `MuMu 安裝路徑` 填寫 MuMu 安裝目錄的根目錄（即包含 `nx_device` 或 `shell` 資料夾的目錄），例如 `C:\Program Files\Netease\MuMuPlayer`。

3. 若正在使用 MuMu 網路橋接，需勾選 `MuMu 網路橋接模式` 並手動填寫多開器內對應的模擬器實例編號，主多開為 `0`。

**MuMu 觸控增強模式**：需官方版 MuMu V6.3.2 及以上，目前方舟專版與國際版均不支援。勾選 `啟用 MuMu 截圖增強` 後，再勾選其下方的 `啟用 MuMu 觸控增強` 即可啟用。
...
5. MuMu 觸控增強：呼叫 MuMu 模擬器外部渲染處理程序的觸控介面，需勾選 `啟用 MuMu 截圖增強` 後才會出現在觸控模式下拉式選單中，不可用時自動回退為 Minitouch（實驗功能）。需官方版 MuMu V6.3.2 及以上，方舟專版與國際版暫不支援。支援 MuMu ｢應用保活｣（背景掛機）場景下操作。

## ADB Lite

由 MAA 獨立實現的 ADB 用戶端，使用 TCP 直接與 ADB Server 通信。相較原版 ADB 可避免頻繁開啟 ADB 處理程序並減少效能開銷，但部分截圖方式不支援。<sup>[PR](https://github.com/MaaAssistantArknights/MaaAssistantArknights/pull/3315)</sup>
...
::: info 實作方式
若需多開模擬器同時執行，可將 MAA 資料夾複製多份，透過 **不同的 MAA 程式**、**同一個 adb.exe**、**不同的連線位址** 來進行連線。
:::
...
1. 啟動**單一**模擬器多開。
2. 開啟工作管理員，找到對應的模擬器處理程序，切換到「詳細資料」索引標籤，右鍵點擊標題列，點選 `選擇行`（或選擇列），勾選 `命令列`。
